    ...
    if (inf.is_open()) {
        ...
        while (getline(inf, line)) {
            ...
            if (line[0] == '/' && line[1] == '/')
               ...
               // read the next parameters
                if (indicator == "USST") {
                    ...
                    cout << indicator<< USSTcap << USSTduration << USSTn << USST_EST_prob[0] << endl;
                    
                } else if (indicator == "EST") {
                    lineStream >> ESTcap >> ESTduration_min >> ESTduration_max >> ESTn >> USST_EST_prob[1] >> zipf_alpha;
                    cout << indicator<< ESTcap << ESTduration_min << ESTduration_max << ESTn << USST_EST_prob[1] << zipf_alpha << endl;
                } else {
            ...
            }else if (round==4)
            {
                ...
                cout <<"prob matrix type:" << prob_matrix_type <<endl;
